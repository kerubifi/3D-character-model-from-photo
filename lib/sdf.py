...
def eval_grid_octree(coords, eval_func, init_resolution=64, threshold=0.05, num_samples=512 * 512 * 512):
    resolution = coords.shape[1:4]
    ...
    while reso > 0:
        ...
        v0 = v[:-1,:-1,:-1]; v1 = v[:-1,:-1,1:]; v2 = v[:-1,1:,:-1]; v3 = v[:-1,1:,1:]
...
# Функции-обёртки для совместимости с ожидаемыми вызовами
def create_grid_simple(resolution, b_min, b_max):
    return create_grid(resolution, resolution, resolution, b_min, b_max)

# Для обратной совместимости
create_grid_legacy = create_grid_simple